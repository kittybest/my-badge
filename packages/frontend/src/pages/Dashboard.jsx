...
export default observer(() => {
  ...
  React.useEffect(() => {
    if (!userContext.userState) {
      setTimeout(() => {
        userContext
          .epochKey(reqRep.nonce ?? 0)
          .then((key) => setReqRep((v) => ({ ...v, epochKey: key })));
      }, 1000);
    } else {
      userContext
        .epochKey(reqRep.nonce ?? 0)
      ...
      <div className="container">
        <div className="info-container">
          ...
          <div className="info-item">
            ...
            <div className="stat">
              {userContext.userState?.calcCurrentEpoch()}
            </div>
          </div>
          <div className="info-item">
          ...
          <div className="info-item">
            <div>Positive</div>
            <div className="stat">
              {userContext.reputation.posRep?.toString()}
            </div>
          </div>
          <div className="info-item">
            <div>Negative</div>
          ...
          <div className="info-item">
            <div>Positive</div>
            <div className="stat">
              {userContext.provableReputation.posRep?.toString()}
            </div>
          </div>
          <div className="info-item">
            <div>Negative</div>
            <div className="stat">
              {userContext.provableReputation.negRep?.toString()}
            </div>
          </div>
          <div>Graffiti</div>
          <div className="graffiti">
            0x{userContext.provableReputation.graffiti?.toString(16)}
          </div>
        </div>
          ...
          <div style={{ display: "flex" }}>
            <div className="action-container">
              ...
              <input
                ...
                onChange={(event) => {
                  setReqRep((v) => ({
                    ...v,
                    graffitiPreImage: event.target.value ?? 0,
                  }));
                }}
              />
              <div className="icon">
              ...
              <p style={{ fontSize: "12px" }}>
                Requesting reputation with epoch key:
              </p>
              <p>{reqRep.epochKey ?? ""}</p>
            ...
            <div className="action-container">
              ...
              <div style={{ margin: "20px 0 20px" }}>
                <Button
                  onClick={async () => {
                    const proof = await userContext.proveReputation(
                      repProofInputs.minRep,
                      repProofInputs.graffitiPreImage
                    );
                    setRepProofInputs({});
                    setRepProof(proof);
                  }}
