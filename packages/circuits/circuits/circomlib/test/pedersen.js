...
describe("Double Pedersen test", function () {
  ...
  before(async () => {
    ...
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "pedersen_test.circom")
    );
  });
  ...
  it("Should pedersen at one first generator", async () => {
    ...
    await circuit.assertOut(w, {
      out: [Fr.toObject(PBASE[0][0]), Fr.toObject(PBASE[0][1])],
    });
  });
  it("Should pedersen at one second generator", async () => {
    ...
    await circuit.assertOut(w, {
      out: [Fr.toObject(PBASE[1][0]), Fr.toObject(PBASE[1][1])],
    });
  });
  ...
  it("Should pedersen all ones", async () => {
    ...
    await circuit.assertOut(w, {
      out: [Fr.toObject(r2[0]), Fr.toObject(r2[1])],
    });
  });
});
