...
describe("Exponentioation test", function () {
  ...
  it("Should generate the Exponentiation table in k=0", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "escalarmulw4table_test.circom")
    );
  ...
  it("Should generate the Exponentiation table in k=3", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "escalarmulw4table_test3.circom")
    );
  ...
  it("Should exponentiate g^31", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "escalarmul_test.circom")
    );
    ...
    const w2 = await circuit.calculateWitness({
      in: Scalar.add(Scalar.shl(Scalar.e(1), 252), Scalar.e(1)),
    });
    ...
    await circuit.assertOut(w2, {
      out: [Fr.toObject(c[0]), Fr.toObject(c[1])],
    });
  }).timeout(10000000);

  it("Number of constrains for 256 bits", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "escalarmul_test_min.circom")
    );
  }).timeout(10000000);
});
