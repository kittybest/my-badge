...
describe("Escalarmul test", function () {
  ...
  before(async () => {
    ...
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "escalarmulfix_test.circom")
    );
  });
  ...
  it("Should generate Same escalar mul", async () => {
    ...
    await circuit.assertOut(w, {
      out: [Fr.toObject(babyJub.Base8[0]), Fr.toObject(babyJub.Base8[1])],
    });
  });

  it("Should generate scalar mul of a specific constant", async () => {
    const s = Scalar.e(
      "2351960337287830298912035165133676222414898052661454064215017316447594616519"
    );
    const base8 = [
      Fr.e(
    ...
    await circuit.assertOut(w, {
      out: [Fr.toObject(expectedRes[0]), Fr.toObject(expectedRes[1])],
    });
  });
