...
describe("Montgomery test", function () {
  ...
  before(async () => {
    ...
    circuitE2M = await wasm_tester(
      path.join(__dirname, "circuits", "edwards2montgomery.circom")
    );
    await circuitE2M.loadSymbols();
    circuitM2E = await wasm_tester(
      path.join(__dirname, "circuits", "montgomery2edwards.circom")
    );
    await circuitM2E.loadSymbols();
    circuitMAdd = await wasm_tester(
      path.join(__dirname, "circuits", "montgomeryadd.circom")
    );
    await circuitMAdd.loadSymbols();
    circuitMDouble = await wasm_tester(
      path.join(__dirname, "circuits", "montgomerydouble.circom")
    );
  ...
  it("Convert Edwards to Montgomery and back again", async () => {
    ...
    w = await circuitE2M.calculateWitness(
      { in: [Fr.toObject(g[0]), Fr.toObject(g[1])] },
      true
    );
