...
describe("Mux4 test", function () {
  ...
  it("Should create a constant multiplexer 4", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "mux4_1.circom")
    );
  ...
  it("Should create a constant multiplexer 3", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "mux3_1.circom")
    );
  ...
  it("Should create a constant multiplexer 2", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "mux2_1.circom")
    );

    const ct4 = [Fr.e("37"), Fr.e("47"), Fr.e("53"), Fr.e("71")];
  ...
  it("Should create a constant multiplexer 1", async () => {
    const circuit = await wasm_tester(
      path.join(__dirname, "circuits", "mux1_1.circom")
    );

    const ct2 = [Fr.e("37"), Fr.e("47")];
