...
describe("Poseidon Circuit test", function () {
  ...
  before(async () => {
    ...
    circuitEx = await wasm_tester(
      path.join(__dirname, "circuits", "poseidonex_test.circom")
    );
  });

  it("Should check constrain of hash([1, 2]) t=6", async () => {
    const w = await circuit6.calculateWitness(
      { inputs: [1, 2, 0, 0, 0] },
      true
    );
  ...
  it("Should check constrain of hash with state and 16 ins and outs", async () => {
    ...
    const w = await circuitEx.calculateWitness({
      inputs: ins,
      initialState: 17,
    });
